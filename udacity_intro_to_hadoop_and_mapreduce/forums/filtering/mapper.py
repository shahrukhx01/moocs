...
def mapper():
    ...
    for line in reader:
        ...
        elif ('?' == line[4][-1] or '.' == line[4][-1]  or '!' == line[4][-1]) and (line[4].count('?')<=1 and line[4].count('.')<=1 and line[4].count('!')<=1):
            if '?' == line[4][-1]  and ( line[4].count('.')<1 and line[4].count('!')<1):
                writer.writerow(line)
            elif '!' == line[4][-1]  and (line[4].count('?')<1 and line[4].count('.')<1):
                writer.writerow(line)
            elif '.' == line[4][-1]   and (line[4].count('?')<1 and line[4].count('!')<1):
                writer.writerow(line)
